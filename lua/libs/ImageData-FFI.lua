...
local _getSize = imagedata_mt.__index.getSize
...
function id_registry:__index(imagedata)
	...
	local size = _getSize(imagedata)
	local pointer = ffi.cast(pixelptr, imagedata:getPointer())
	local p = {width=width, height=height, size=size, pointer=pointer}
	self[imagedata] = p
	return p
end
...
-- FFI version of ImageData:clear, with no thread-safety.
local function ImageData_FFI_clear(imagedata)
	local p = id_registry[imagedata]
	local idw, idh = p.width, p.height	
	local pixels = p.pointer
	local size = p.size
	ffi.fill(pixels, size)	
end
...
imagedata_mt.__index.clear = ImageData_FFI_clear
