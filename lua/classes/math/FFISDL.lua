...
local FFISDL = class('FFISDL')
local instance = FFISDL()
...
function FFISDL.extend() 
  error('Cannot extend from a FFIMatrix')
end

function FFISDL:init()	
end

function FFISDL:getInstance()
  return instance
end

function FFISDL:showError(msg)
	local sdlMsg = ffi.string(sdl.SDL_GetError())
	if self.window then
		sdl.SDL_DestroyWindow(self.window)
	end
	if self.backBuffer then
		sdl.SDL_FreeSurface(self.backBuffer)
	end
...
function FFISDL:start(w, h)
	if sdl.SDL_Init(sdl.SDL_INIT_VIDEO) ~= 0 then
		self:showError('SDL_Init')
	end
	
	self.window = sdl.SDL_CreateWindow('Space Game', 100, 100, w, h, sdl.SDL_WINDOW_OPENGL)
	if not self.window then
		self:showError('SDL_CreateWindow')
	end
	
	self.screen = sdl.SDL_GetWindowSurface(self.window)
	if not self.screen then
		self:showError('SDL_GetWindowSurface')
	end
	
	self.backBuffer = sdl.SDL_CreateRGBSurface(0,w,h,32,0,0,0,0)
	if not self.backBuffer then
		self:showError('SDL_CreateRGBSurface')
	end
	
	local t = ffi.new('uint8_t[?]', w * h * 4)
	for i = 0, w * h * 4, 4 do
		t[i] = 255
		t[i+3] = 64
	end
	ffi.copy(self.backBuffer.pixels, t, w * h * 4)
	
	self.fullScreenRect = ffi.new('SDL_Rect')
	self.fullScreenRect.x = 0
	self.fullScreenRect.y = 0
	self.fullScreenRect.w = w
	self.fullScreenRect.h = h		
end

function FFISDL:blit()
	if sdl.SDL_LowerBlit(self.backBuffer, self.fullScreenRect, self.screen, self.fullScreenRect) ~= 0 then
		self:showError('SDL_LowerBlit')	
	end
    	
	if sdl.SDL_UpdateWindowSurface(self.window) ~= 0 then
		self:showError('SDL_UpdateWindowSurface')	
	end
end

function FFISDL:stop()
